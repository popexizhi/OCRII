...
class char_img():
	...
	def get_move_range(self):
		return self.move_range #返回设置的检查变化位置
...
def mave_xy(filepath="1.gif"):
	...
	reslist_y=[]
	xy_list=[]
	(x_list,y_list)=char_xyMax("char-code_mav/") #字库位置
	for i in x_list:
	...
	xy_list.sort()
	#print xy_list[-1]
	#判断使用者，有完成10**move_range
	move_range=10 ** (use_diff.get_move_range()*2 + 1 ) 
	#print "move_range is %d" % move_range
	if (move_range == reslist_x[-1][0]): 
		return reslist_x[-1]
	if (move_range == reslist_y[-1][0]):
		return reslist_y[-1]
